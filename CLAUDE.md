...
## Parked / unmerged work

- **PR #11 — Phase 4 part 2 (tax-lot UI + classification + tax estimates).** Built and reviewed; intentionally left open on branch `feat/tax-lots-ui` and NOT merged. The owner has no holdings to test the feature against yet and will merge when actual tax-lot tracking begins. Migration `0004_user_settings_tax_rates.sql` ships in this PR and **has NOT been applied to the prod DB** — the corresponding column additions on `user_settings` (`effective_lt_tax_rate_pct`, `effective_st_tax_rate_pct`) only exist in the PR branch's `schema.sql`. When the time comes: apply 0004 first, then merge `--no-ff`. Do not assume the columns exist in prod until both steps are done.
- The Phase 4 part 1 data layer (PR #9, `holding_lots` table + migration 0003 + ARCHITECTURE invariant + classification contract) IS live in prod; only the application surface that consumes those lots is parked.
